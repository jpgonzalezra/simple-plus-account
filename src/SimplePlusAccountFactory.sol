...
contract SimplePlusAccountFactory {
    ...
    function createAccount(address owner, uint256 salt) public returns (SimplePlusAccount ret) {
        ...
        ret = SimplePlusAccount(
            payable(
                new ERC1967Proxy{ salt: bytes32(salt) }(
                    address(accountImplementation), abi.encodeCall(SimplePlusAccount.initialize, (owner))
                )
            )
        );
    }
        ...
        return Create2.computeAddress(
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    abi.encode(address(accountImplementation), abi.encodeCall(SimplePlusAccount.initialize, (owner)))
                )
            )
        );
    }
